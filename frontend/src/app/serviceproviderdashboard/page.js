...
const ServiceProviderDashboard = () => {
    ...
    const verifyToken = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          socket.emit(
            "joinServiceProvider",
            response.data.serviceprovider.email
          );
        }
      } catch (err) {
  ...
  const handleAccept = (requestId, customerlocation, servicetype, name) => {
    if (!customerlocation) {
        console.error("customerlocation is undefined or empty");
        return;
    }

    // Ensure customerlocation is a string before splitting
            ...
            {requests.length > 0 ? (
              requests.map((req, index) => (
                ...
                >
                  ...
                  <p>
                    <strong>Location:</strong> {req.customerLocation.join(", ")}
                  </p>
                  <div className="mt-2">
                    <button
                      onClick={() =>
